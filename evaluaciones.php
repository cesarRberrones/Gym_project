...
// ==================== LISTA DE SOCIOS PARA SELECT (CORREGIDO) ====================
$socios_lista = null;
if (!$es_socio) {
    if ($es_admin) {
        // Admin: ve todos los socios
        ...
        // Entrenador: solo sus socios asignados
        $id_entrenador_query = $conn->query("SELECT ID_Entrenador FROM entrenadores WHERE ID_Usuario = " . $_SESSION['usuario_id']);
        if ($id_entrenador_query && $row = $id_entrenador_query->fetch_assoc()) {
            $id_entrenador = $row['ID_Entrenador'];
            $socios_lista = $conn->query("SELECT DISTINCT s.ID_Socios, u.Nombre 
                                          FROM socios s 
                                          INNER JOIN usuarios u ON s.ID_Usuario = u.ID_Usuario
                                          INNER JOIN planes_rutinas pr ON s.ID_Socios = pr.ID_Socio
...
<body>
                        ...
                        <table class="table table-vcenter">
                            ...
                            <tbody>
                                ...
                                <?php if(empty($comidas_hoy)): ?>
                                  <td><td colspan="<?php echo $es_socio ? '4' : '5'; ?>" class="text-center">No hay comidas registradas hoy<\/td><\/tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
    ...
    <script>
        ...
        function agregarComentario(id, alimento) {
            ...
            var modalElement = document.getElementById('modalComentario');
